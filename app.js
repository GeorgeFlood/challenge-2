...
const arrJumble = array => {
let jumble = array[Math.floor(Math.random() * array.length)];
return jumble;	
}

console.log(arrJumble(quotes));

button1.addEventListener('click', (e) => {
edit.innerHTML = `${arrJumble(jokes)}`;
  });

button2.addEventListener('click', (word) => {
edit.innerHTML = `${arrJumble(quotes)}`;
});

button3.addEventListener('click', (word) => {
edit.innerHTML = `${arrJumble(sentences)}`;
});
